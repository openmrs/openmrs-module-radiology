    ...
    <table id="radiologyOrdersTable" cellspacing="0" width="100%" class="display nowrap">
      ...
      <tbody id="radiologyOrdersTableBody">
        <c:forEach items="${radiologyOrders}" var="radiologyOrder">
          <tr data-child-order_id="${radiologyOrder.orderId}" data-child-physician="${radiologyOrder.orderer.name}"
            data-child-status="<spring:message code="radiology.${radiologyOrder.study.scheduledStatus}"
            text="${radiologyOrder.study.scheduledStatus}" />"
            data-child-instructions="${radiologyOrder.instructions}">
            <td class="details-control"></td>
            <td><a href="radiologyOrder.form?orderId=${radiologyOrder.orderId}">${radiologyOrder.orderId}</a></td>
            ...
            <td><a href="/openmrs/patientDashboard.form?patientId=${radiologyOrder.patient.patientId}">${radiologyOrder.patient.personName}</a></td>
...
<c:if test="${not empty exceptionText}">
  <span id="errorSpan" class="error"> <spring:message code="${exceptionText}" arguments="${invalidValue}" />
  </span>
</c:if>
