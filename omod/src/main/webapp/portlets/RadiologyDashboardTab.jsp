...
<openmrs:htmlInclude
	file="/moduleResources/radiology/radiology.css" />
<openmrs:htmlInclude
	file="/moduleResources/radiology/js/jquery.dataTables.min.js" />
<%@ include
	file="/WEB-INF/view/module/radiology/resources/js/orderList.js" %>
<openmrs:htmlInclude
	file="/moduleResources/radiology/js/sortNumbers.js" />
<openmrs:htmlInclude
	file="/moduleResources/radiology/css/ColVis.css" />
<openmrs:htmlInclude
	file="/moduleResources/radiology/js/ColVis.min.js" />
                                    ...
                                    <c:forEach items="${orderList}" begin="0" end="${matchedOrdersSize}"
                                            ...
                                            <tr>
                                                    <td style="text-align: center"><c:if test="${empty obsId}">
                                                            ...
                                                            <a href="module/radiology/portlets/radiologyObsDashboard.form?orderId=${order.orderId}" style="cursor:pointer;" class="viewRadiologyObservations">${status.count}</a>
                                                            </c:if>
                                                            </td>
...
var $j=jQuery.noConflict();                
	$j(document).ready(function() {
		$j('#viewRadiologyObservationsPopup').dialog({
			autoOpen: false,
			modal: true,
			position: top,
		...
		$j('.viewRadiologyObservations').click(function(e) {
                         e.preventDefault();
                         var url=$j(this).attr('href');                            
                         $j('#viewRadiologyObservationsPopup').load(url);
			 $j('#viewRadiologyObservationsPopup').dialog('open');                                                 
		});						            
