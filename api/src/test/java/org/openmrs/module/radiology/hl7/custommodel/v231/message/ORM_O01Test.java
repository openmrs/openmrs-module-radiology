...
public class ORM_O01Test {
	...
	/**
	 * Tests the ORM_O01.getZDS()
	 * 
	 * @throws HL7Exception
	 * @see {@link ORM_O01#getZDS()}
	 */
	@Test
	@Verifies(value = "should return the ZDS segment", method = "getZDS()")
	public void getZDS_shouldReturnTheZDSSegment() throws HL7Exception {
	 ...
	 */
	@Test
	@Verifies(value = "should create a new ORM_O01 instance", method = "ORM_O01()")
	public void ORM_O01_shouldCreateANewORM_O01Instance() throws HL7Exception {
		
		String hl7blob = "MSH|^~\\&|OpenMRSRadiologyModule|OpenMRS|||20130827154500||ORM^O01||P|2.3.1\r"
		...
		ORM_O01 ormCustom = new ORM_O01();
		
		MSH mshCustom = ormCustom.getMSH();
		...
		PID pid = ormCustom.getPIDPD1NTEPV1PV2IN1IN2IN3GT1AL1().getPID();
		...
		ORC orc = ormCustom.getORCOBRRQDRQ1ODSODTRXONTEDG1RXRRXCNTEOBXNTECTIBLG().getORC();
		...
		OBR obr = ormCustom.getORCOBRRQDRQ1ODSODTRXONTEDG1RXRRXCNTEOBXNTECTIBLG()
		...
		ZDS zds = ormCustom.getZDS();
		zds.getStudyInstanceUID().getPointer().setValue("1.2.826.0.1.3680043.8.2186.1.1.1");
		zds.getStudyInstanceUID().getApplicationID().getNamespaceID().setValue("1");
		zds.getStudyInstanceUID().getTypeOfData().setValue("Application");
		zds.getStudyInstanceUID().getSubtype().setValue("DICOM");
		
		assertThat(PipeParser.encode(ormCustom, encodingCharacters), is(hl7blob));
	}
