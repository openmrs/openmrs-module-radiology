...
class MultiFindRSP implements DimseRSP {
    ...
    public MultiFindRSP(DcmRcv dcmOF, DicomObject keys, DicomObject rsp, File source) {
        this.dcmrcv = dcmOF;
        this.keys = keys;
        ...
        if (files == null) {
            ...
            try {
                while (cur < files.length) {
                    image = dcmrcv.load(files[cur++]);
                    if (image.matches(keys, true)) {
                        // always return Specific Character Set
                        if (!image.contains(Tag.SpecificCharacterSet))
                            image.putNull(Tag.SpecificCharacterSet, VR.CS);
                        rsp.putInt(Tag.Status, VR.US, image.containsAll(keys) 
    ...
    public DicomObject getDataset() {
        return image != null ? image.subSet(keys) : null;
    }
