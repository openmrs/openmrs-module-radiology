...
public class RadiologyStudyEditor extends PropertyEditorSupport {
    ...
    public void setAsText(String text) throws IllegalArgumentException {
        RadiologyStudyService radiologyStudyService = Context.getService(RadiologyStudyService.class);
        if (StringUtils.hasText(text)) {
            try {
                setValue(radiologyStudyService.getStudyByStudyId(Integer.valueOf(text)));
            }
            catch (Exception ex) {
                RadiologyStudy radiologyStudy = radiologyStudyService.getStudyByStudyInstanceUid(text);
    ...
    public String getAsText() {
        RadiologyStudy radiologyStudy = (RadiologyStudy) getValue();
        if (radiologyStudy == null) {
            return "";
        } else {
