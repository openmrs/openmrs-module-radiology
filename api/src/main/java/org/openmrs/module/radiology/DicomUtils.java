...
public class DicomUtils {
	...
	public static String findDCM(Order o, int[] tag) throws IOException {
		...
		String status = "?";
		String[] list = f.list();
		SpecificCharacterSet scs = new SpecificCharacterSet(
				Utils.specificCharacterSet());
		int arg1 = 0;
		for (String string : list) {
			...
			try {
				...
				if ((Utils.studyPrefix() + o.getOrderId().toString())
						.compareTo(d.get(Tag.ScheduledStepAttributesSequence)
								.getDicomObject().get(Tag.StudyInstanceUID)
								.getValueAsString(scs, arg1)) == 0) {
	...
	public static void write(Order o, Study s, File file)
		...
		workitem.putString(Tag.SpecificCharacterSet, VR.CS,
				Utils.specificCharacterSet());
		...
		try {
			workitem.putString(
					...
					VR.PN,
					o.getOrderer().getPersonName().getFullName()
							.replace(' ', '^'));
		} catch (Exception e) {
		...
		try {
			workitem.putString(Tag.PatientName, VR.PN, o.getPatient()
					.getPersonName().getFullName().replace(' ', '^'));
			workitem.putString(Tag.PatientID, VR.LO, o.getPatient()
					.getPatientIdentifier().getIdentifier());
			workitem.putString(Tag.PatientBirthDate, VR.DA,
					Utils.plain(o.getPatient().getBirthdate()));
			workitem.putString(Tag.PatientSex, VR.CS, o.getPatient()
					.getGender());
		} catch (Exception e) {
		...
		workitem.putString(Tag.StudyInstanceUID, VR.UI,
				Utils.studyPrefix() + s.getId());
		try {
			workitem.putString(Tag.RequestingPhysician, VR.PN, o.getOrderer()
					.getPersonName().getFullName().replace(' ', '^'));
		} catch (Exception e) {
			debug("Not saving requesting physician");
		}
		try {
			workitem.putString(Tag.RequestedProcedureDescription, VR.LO,
					o.getInstructions());
		} catch (Exception e) {
		...
		spss.putString(Tag.Modality, VR.CS,
				Modality.values()[s.getModality()].toString());
		spss.putString(Tag.RequestedContrastAgent, VR.LO, "");
		spss.putString(Tag.ScheduledStationAETitle, VR.AE, Utils.aeTitle());
		try {
			spss.putString(Tag.ScheduledProcedureStepStartDate, VR.DA,
					Utils.plain(o.getStartDate()));
			spss.putString(Tag.ScheduledProcedureStepStartTime, VR.TM,
					Utils.time(o.getStartDate()));
			spss.putString(Tag.ScheduledPerformingPhysicianName, VR.PN, "!");
			spss.putString(Tag.ScheduledProcedureStepDescription, VR.LO,
					o.getInstructions());
		} catch (Exception e) {
		...
		spss.putString(Tag.ScheduledProcedureStepID, VR.SH,
		...
		spss.putString(Tag.ScheduledProcedureStepStatus, VR.CS,
				ScheduledStatuses.string(s.getScheduledStatus(), false));
		workitem.putNestedDicomObject(Tag.ScheduledProcedureStepSequence, spss);

		workitem.putString(Tag.RequestedProcedureID, VR.SH,
				String.valueOf(s.getId()));
		workitem.putString(Tag.RequestedProcedurePriority, VR.SH,
				Study.Priorities.string(s.getPriority(), false));
		workitem.putString(Tag.PatientTransportArrangements, VR.LO, "");
		workitem.putString(
				Tag.ConfidentialityConstraintOnPatientDataDescription, VR.LO,
				"");
	...
	public static void writeMpps(DicomObject o) {
		try {
			...
			int[] studyUIDPath = { Tag.ScheduledStepAttributesSequence,
					Tag.StudyInstanceUID };
			String studyUID = o.get(studyUIDPath[0]).getDicomObject()
					.get(studyUIDPath[1]).getValueAsString(scs, 0);
                        
			String[] uidSplit = studyUID.split("[.]");
			int id = Integer.parseInt(uidSplit[uidSplit.length - 1]);
			Study s = service().getStudy(id);
			debug(s.toString());
			String pStatus = o.get(Tag.PerformedProcedureStepStatus)
...
//                            "ZDS|1.2.826.0.1.3680043.8.2186.1.2|";
        ...
        public static int sendHL7Worklist(String hl7blob)
        {
            ...
            String input[] = {"-c",serverIP+":2575",hl7blob};
            //String input[]={"--help"};
